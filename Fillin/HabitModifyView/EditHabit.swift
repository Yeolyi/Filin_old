...
struct EditHabit: View {
    ...
    init(targetHabit: Habit) {
        ...
        self._requiredTime = State(initialValue: String(targetHabit.requiredSecond))
    }
    ...
    func saveAndExit() {
        ...
        guard let requiredTime = Int16(self.requiredTime), isRequiredTime else {
            targetHabit.requiredSecond = 0
            CoreDataManager.save(managedObjectContext)
            self.presentationMode.wrappedValue.dismiss()
            return
        }
        targetHabit.requiredSecond = requiredTime
        CoreDataManager.save(managedObjectContext)
        self.presentationMode.wrappedValue.dismiss()
    }
    var body: some View {
