...
struct CircleProgress<Content: View>: View {
    ...
    let content: Content
    var isRing: Bool { !ringDatas.isEmpty }
    let customFrame: CGFloat?
    @State var isAnimation = false
    @Environment(\.colorScheme) var colorScheme
    init(_ ringDataTuples: [(progress: Double, color: Color)?], _ customFrame: CGFloat? = nil, @ViewBuilder content: @escaping () -> Content) {
